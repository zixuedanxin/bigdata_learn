...
class KafkaSink[K, V](createProducer: () => KafkaProducer[K, V]) extends Serializable {
  ...
  def send(topic: String, partitionNum: Integer, key: K, value: V, callback: mycallback) =
    try {
      producer.send(new ProducerRecord[K, V](topic, partitionNum, key, value), new mycallback).get()
    } catch {
      case ex: InterruptException => print("inter")
    }
}
