...
object StructuredOrderExt {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sessionUpdates = events
      ...
      .mapGroupsWithState[orderAggInfo, orderAggUpdate](GroupStateTimeout.ProcessingTimeTimeout) {
      case (storeId: String, events: Iterator[OEvent], state: GroupState[orderAggInfo]) =>
        println("events==" + events)
        // 如果时间超时，更新缓存
        if (state.hasTimedOut) {
          println("超时:" + state.hasTimedOut)
          val finalUpdate =
            orderAggUpdate(storeId, state.get.num, state.get.money, state.get.durationMs, expired = true)
          ...
          println(events.map(_.money).toSeq)
          var moneySum = 0.0
          val moneySums = events.map(_.money).toSeq
          if (moneySums.size > 0) {
            moneySum = moneySums.reduce(_ + _)
          }
          val updatedSession = if (state.exists) {
            val oldOrder = state.get
            orderAggInfo(
              oldOrder.num + timestamps.size,
              oldOrder.money + moneySum,
              oldOrder.startTimestampMs,
              math.max(oldOrder.endTimestampMs, timestamps.max))
          } else {
            orderAggInfo(timestamps.size, moneySum, timestamps.min, timestamps.max)
          }
          state.update(updatedSession)
          // Set timeout such that the session will be expired if no data received for 10 seconds
          state.setTimeoutDuration("10 seconds")
          orderAggUpdate(storeId, state.get.num, state.get.money, state.get.durationMs, expired = false)
        }
    }
...
case class OEvent(orderId: String, otype: Int, storeId: String, money: Double, orderDate: String, timestamp: Timestamp)
