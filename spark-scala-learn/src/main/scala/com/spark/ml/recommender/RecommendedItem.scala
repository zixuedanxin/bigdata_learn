package com.spark.ml.recommender
...
class RecommendedItem {
  ...
  /**
    ...
    * @param r_number 推荐数量
    * @return RDD[UserRecomm] 返回用户推荐的物品
    */
  def Recommend(items_similar: RDD[ItemSimi],
    ...
    // 矩阵计算 --- i行与j列join
    val rdd_app1_R2: RDD[(String, ((String, Double), (String, Double)))] = rdd_app1_R1.map(f => (f._1, (f._2, f._3)))
    ...
    // 矩阵计算 --- 用户：对结果过滤已有的物品
    val rdd_app1_R5: RDD[(String, (String, Double))] = rdd_app1_R4
      .leftOuterJoin(
        user_prefer1
          .map(f => ((f._1, f._2), 1)))
  ...
  def Recommend(items_similar: RDD[ItemSimi],
    ...
    val rdd_app1_R4 = rdd_app1_R3.reduceByKey((x, y) => x + y)
    // 矩阵计算 --- 用户：对结果过滤已有物品
    val rdd_app1_R5 = rdd_app1_R4
      .leftOuterJoin(user_prefer1.map(f => ((f._1, f._2), 1)))
      .filter(f => f._2._2.isEmpty)
      .map(f => (f._1._1,(f._1._2, f._2._1)))
    // 矩阵计算 --- 用户：对用户结果排序，过滤
