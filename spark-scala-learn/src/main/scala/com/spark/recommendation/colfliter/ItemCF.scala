package com.spark.recommendation.colfliter
...
class ItemCF {
  ...
  def computeJaccardSimWithDF(sc: SparkContext,
    ...
    val rdd = sqlContext.read.parquet(featurePath).select("sparkml/features")
  ...
  def predictBySimVecWithDF(sc: SparkContext,
    ...
    val sqlContext = new SQLContext(sc)
    sqlContext.read.parquet(featurePath).select("id", "sparkml/features")
      .rdd.map(x=>{
        val p = x(0).toString
