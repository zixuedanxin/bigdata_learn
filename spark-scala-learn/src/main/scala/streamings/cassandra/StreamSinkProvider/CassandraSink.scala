package streamings.cassandra.StreamSinkProvider
...
import streamings.kafka.KafkaMetadata
import streamings.cassandra.CassandraDriver
import streamings.log.LazyLogger
...
class CassandraSink() extends Sink with LazyLogger {
  ...
  private def saveToCassandra(df: DataFrame) = {
    ...
    ds.rdd.saveToCassandra(CassandraDriver.namespace,
      ...
      SomeColumns("title", "artist", "streamings/radio", "count")
    )
