...
object sync_mysql2hbase_prd {
  //mysql配置信息
  val url1 = "jdbc:mysql://192.168.201.62:3306/information_schema"
  val url2 = "jdbc:mysql://192.168.201.62:3307/information_schema"
  val user = ""
  val password = ""
  var tb_key = Map[String, String]()
  tb_key = get_mysql_metadata(url1, user, password)
  tb_key = tb_key ++ get_mysql_metadata(url2,user,password)

  val diu_type_in_hbase = Map("delete" -> 0, "insert" -> 1, "update" -> 2)
  val table_map = get_needsync_tables()
  val need2synctables =  table_map.keys.toList
  var table_stamp = Map[String,List[String]]()//{hbase_tablename:[clo1,clo2,...],...}
  for(i <- table_map.keys) {if(table_map.get(i).mkString != "null") { table_stamp += (i -> table_map.get(i).toString.split(",").toList)}}

  //hbsae配置信息
  val leizuname = "datas" //配置hbsae表中的列簇名称，目前仅支持一个列簇
  val zkips = "192.168.210.11,192.168.210.13,192.168.210.16,192.168.210.10,192.168.210.18"
  val zkipports = "192.168.210.16:2181,192.168.210.13:2181,192.168.210.11:2181,192.168.210.10:2181,192.168.210.18:2181"
  ...
  //kafka配置信息
  val topics = Set("maxwell3partitions")
  val group_id = "sync2hbase"
  val brokers = "192.168.210.11:9092,192.168.210.14:9092,192.168.210.17:9092"
  val offsettable = "sync2hbasekafkaoffset" //kafkaoffset消费信息存储的hbase 表名
  //设置日志级别
  Logger.getLogger("org").setLevel(Level.ERROR)


  //时间匹配
  def judge_time(x:String):String= {
    val p = "\\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2}:\\d{2}".r
    val rs = p findAllIn x
    if(rs.isEmpty){return "null"}
    else {return "notnull"}
  }

  //将UTC时间转化成CST时间
  def make_utc2cst(ts:String):String= {
    //    val nowdatetime = NowDate()
    var df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
    var dt = df.parse(ts)
    val calendar = Calendar.getInstance()
    calendar.setTime(dt)
    calendar.add(Calendar.HOUR_OF_DAY, 8)
    val str_date = (new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")).format(calendar.getTime())
    return str_date
  }

  //读取配置文件,获取需要同步的表及该表中类型为timestamp的字段
  def get_needsync_tables():Map[String,String]= {
    var table_map = Map[String, String]()
    val path_file = System.getProperty("user.dir") + "/sync_table_config.txt"
    val file = Source.fromFile(path_file)
    for (i <- file.getLines) {
      if (!i.contains("数据库")) {
        val table_list = i.split(";")
        if (table_list.length > 1) {
          table_map += (table_list(0).mkString -> table_list(1).mkString)
        }else{
          table_map += (table_list(0).mkString -> "null")
        }
      }
    }
    file.close()
  ...
  def get_mysql_metadata(url: String, user: String, password: String): Map[String, String] = {
    ...
    val sQLStatement = "select concat(TABLE_SCHEMA,':',TABLE_NAME) tb,GROUP_CONCAT(COLUMN_NAME) key_cols from information_schema.key_column_usage where CONSTRAINT_NAME='PRIMARY' and TABLE_SCHEMA <> 'mysql' and  COLUMN_NAME <> 'sina_id' GROUP BY TABLE_SCHEMA ,TABLE_NAME"
  ...
  def get_kafka_offset(tablename: String, group_id: String, topic: String, parts: collection.mutable.Buffer[Int]): collection.mutable.Buffer[Map[String, String]] = {
    ...
    for (k <- parts) {
      rowkeys += topic + k.toString + group_id
    }
    val table = new HTable(hconf, tablename)
  ...
  def iterator_fc(iterator: Iterator[String]): Iterator[String] = {
    ...
    while (iterator.hasNext) {
      ...
      if (need2synctables.contains(u_tablename)) {
        if (!hbase_exist_tables.contains(u_tablename)) {
          make_hbase_namespace_table(u_tablename)
  ...
  def format_fc(iterator: Iterator[String]): Iterator[String] = {
    ...
    while (iterator.hasNext) {
      ...
      val u_tablename = dict_data("table").toString
      //格式化数据
      val myrowkey_cols = tb_key(u_tablename).split(",").sorted
      ...
      def mkhbaseput(a:String):String={
        //使用splitstr1将list合并成字符串
        var datav: String = " " //防止有的值为null
        val x = jdata2.getString(a.toString).mkString
        if (x.length > 0) {
          if(! table_stamp.get(u_tablename).toString.contains(a)) {
            datav = x}
          else {
            if(judge_time(x) == "notnull") {
              datav = make_utc2cst(x)} else {datav = x}
          }
        }
        // 表名，rowkey，列簇，列，值
        val ii = List(u_tablename, u_thisrowkey, u_cols, a, datav).mkString(splitstr1)
        return ii
      }
      //使用splitstr2合并list
      val xx = jdata2_keys.map(i => mkhbaseput(i.toString)).toList.mkString(splitstr2)//map执行后返回一个迭代器
  ...
  def main(args: Array[String]): Unit = {
    ...
    val zk = new ZooKeeper(zkipports, 500, null)
    val s = zk.getChildren(s"/brokers/topics/${topics.mkString}/partitions", null)
    ...
    if (kafka_offset_info.length > 0) { //可以从hbase获取到offset
      for (i <- 0 to kafka_offset_info.length - 1) {
        if (kafka_offset_info(i).get("offsets").mkString.toInt > 1000) {
          partiton_offset.append((i, kafka_offset_info(i).get("offsets").mkString.toInt - 1000))
        } else {
    ...
    val conf = new SparkConf().setAppName("streaming_mysql2hbase")//.setMaster("local[*]")
    conf.set("spark.serializer", "org.apache.spark.serializer.KryoSerializer")
    val ssc = new StreamingContext(conf, Seconds(3))
    val kafkaParams = Map[String, String]("metadata.broker.list" -> brokers, "serializer.class" -> "kafka.serializer.StringEncoder")

    var messages: InputDStream[(String, String)] = null
    val topic = Array("hydd_log","testlog","df")
...
//    }
    ...
    //格式化字符串
    val lines3 = lines2.mapPartitions(format_fc).flatMap(line => {
      line.toString.split(splitstr2)
    ...
    result.foreachRDD(rdd => {
      if (! rdd.isEmpty()) {
        rdd.saveAsNewAPIHadoopFile("/user/yfgtest", classOf[ImmutableBytesWritable], classOf[Put], classOf[MultiTableOutputFormat], jobConf)
      }
    })
    ...
    messages.transform { rdd =>
      ...
      for (o <- offsetRanges) {
        val numoffsets = o.untilOffset - o.fromOffset
        println(s"${NowDate()} ${o.topic} ${o.partition} ${o.fromOffset} ${o.untilOffset} ${numoffsets}")
        val data = Map("tablename" -> offsettable, "group_id" -> group_id, "topic" -> o.topic.toString, "parts" -> o.partition.toString, "offsets" -> o.untilOffset.toString, "update_time" -> NowDate())
