...
object WordCount extends App {
  ...
  val line = sc.textFile("src/main/resource/data/text/people.txt")
  val word = line.flatMap(line => line.split(" "))
  val counts = word.map(word => (word, 1)).reduceByKey(_+_)
 ...
 // val sc = new SparkContext(conf) //创建SparkContext对象，通过传入SparkConf实例来定制Spark运行的具体参数和配置信息
  ...
  val lines = sc.textFile("data/word.txt", 1) //读取本地文件并设置为一个Partion
  /**
  ...
  /**
    ...
    */
  val wordCountsOdered =
    pairs.reduceByKey(_ + _, 1000)
    .map(pair => (pair._2, pair._1))
    .sortByKey(false)
    .map(pair => (pair._2, pair._1)) //对相同的Key，进行Value的累计（包括Local和Reducer级别同时Reduce）
