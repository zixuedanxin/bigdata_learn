...
/**
  * Created by zhaohui on 18-7-26.
  * 利用线性回归模型对数据进行回归预测
  */

object SparkMllibLinearRegression {
  def main(args: Array[String]): Unit = {
    ...
    val assembler = new VectorAssembler().setInputCols(colArray).setOutputCol("sparkml/features")
    ...
    val assembler_for_predict = new VectorAssembler().setInputCols(colArray_for_predict).setOutputCol("sparkml/features")
    ...
    println("输出预测结果")
    val predict_result: DataFrame = predictions.selectExpr("sparkml/features", "Murder", "round(prediction,1) as prediction")
