...
/**
  *  Spark商业案例书稿第13章： Dataset开发实战企业人员管理系统应用案例代码
  * 版权：DT大数据梦工厂所有
  * 时间：2017年1月1日；
  **/
object DatasetOps {
  ...
  def main(args: Array[String]) {
    ...
    val rdd = spark.sparkContext.makeRDD(-1000 until 1000, 10)
    var sum11 = 0
    for (depth <- 1 until 10) {
       sum11 = rdd.treeReduce(_ + _, depth)
      println("tree======================"+sum11)
    }

    println("tree==========total ============"+sum11)

    val personsDF: DataFrame = spark.read.json("data/people.json")
    val personScoresDF = spark.read.json("data/peopleScores.json")
    ...
    personsDS.groupBy($"name", $"age").agg(concat($"name", $"age").as("concat")).show

    println("使用col算子选择列 ：")
    personsDS.show(10)
    personScoresDS.show(10)
    ...
    val repartitionDs = personsDS.repartition(2)
    println("repartition设置分区数：" + repartitionDs.rdd.partitions.size)

    println("使用coalesce算子设置分区：")
    val coalesced: Dataset[Person] = repartitionDs.coalesce(4) // 只能减少分区不能增加分区数 没有shuffle
