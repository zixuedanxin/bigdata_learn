...
object Ex6_Accumulators {
  ...
  class StringSetAccumulator extends AccumulatorV2[String, java.util.Set[String]] {
    ...
    override def isZero: Boolean = _set.isEmpty  // 是否为空
    ...
    override def reset(): Unit = _set.clear()  // 置空对象
    override def add(v: String): Unit = _set.add(v) // 添加
