...
object Hbase_obj {
  ...
  def test(): Unit ={
    ...
    try {
      ...
      insertHTable(connection,"blog","author","engish","003","python for me")
      insertHTable(connection,"blog","author","chinese","002","C++ for china")
      //删除记录
  ...
  def main(args: Array[String]): Unit = {
    ...
    val listtable=admin.listTableNames()
    //admin.listTableNames().foreach(a => println(a))
    println(TableName.valueOf("yfgtest:hua"))
    try{
      val userTable=TableName.valueOf("yfgtest:hua")
      table=connection.getTable(userTable)
      val s=new Scan()
      //s.addColumn(family.getBytes(),column.getBytes())
