...
public class log_kafka_monitor {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        for (int i = 0; i < 5000; i++){
            ...
            rows.put("act_cntnt","test"+String.valueOf(nows%1000));
            rows.put("act_type","a");
            rows.put("crt_dtm",String.valueOf(nows));
            System.out.println(new JSONObject(rows).toString());
            producer.send(new ProducerRecord<String, String>(topic, String.valueOf(nows), new JSONObject(rows).toString()));
            rows.clear();
            Thread.sleep(20000);
        }
